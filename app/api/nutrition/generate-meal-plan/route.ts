...
export async function POST(request: Request): Promise<Response> {
  ...
  // Use profileData from request body if provided (from Zustand),
  // otherwise fall back to fetching from Supabase
  let profile: ProfileContext | null = null;

  if (profileData && typeof profileData === 'object') {
    profile = profileData as ProfileContext;
  } else {
  ...
  if (!profile) return Response.json({ suggestions: [] }, { status: 200 });

  // Fetch today's consumed macros from Supabase
  const supabase = createServerClient();
  ...
  // Use remainingMacros from body if provided, else compute from profile
  const remaining: RemainingMacros = remainingMacros && typeof remainingMacros === 'object'
    ? remainingMacros as RemainingMacros
    : {
  ...
  const prefStr = Array.isArray(foodPreferences) && foodPreferences.length
    ? `Food preferences: ${(foodPreferences as string[]).join(', ')}.` : '';
  const allergyStr = Array.isArray(allergies) && allergies.length
    ? `Allergies to avoid: ${(allergies as string[]).join(', ')}.` : '';
...
Suggest practical, realistic meals. All numeric values must be non-negative integers.`;
  ...
  try {
    ...
    if (!openAIResponse.ok) return new Response('Meal plan generation failed', { status: 502 });
    ...
    catch { return new Response('Meal plan generation failed', { status: 502 }); }

    if (!isValidMealSuggestions(parsed)) return new Response('Meal plan generation failed', { status: 502 });
